...
// write weather mask to a FIFO, max once/second
void run(const char* out_file_name) {
    ...
    int mask_pipe = open(out_file_name, O_WRONLY); // open FIFO for writing
    if (mask_pipe) {
        while (1) {
            try {
                ...
                const std::string mask_s = std::to_string(
                    unsigned(
                        (intrp.is_clear << 0) | (intrp.is_sunny << 1) | (rec.is_sun_up << 2) |
                        (intrp.is_windy << 3) | (intrp.is_rainy << 4) | (intrp.is_snowy << 5) |
                        (intrp.is_hot << 6) | (intrp.is_cold << 7) | (intrp.is_freezing << 8) |
                        (intrp.is_warm << 9)
                    )
                ) + "\n";
                write(mask_pipe, mask_s.c_str(), mask_s.length());
